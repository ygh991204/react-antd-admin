...
const svgNames = Object.keys(modulesFiles)
  ...
  .reduce((modules, modulePath) => {
    modules.push(modulePath.replace(/^\.\/(.*)\.\w+$/, '$1').replace('../../../assets/icons/', '').replace('.svg', ''))
    return modules
  }, [])
    ...
    <Card bordered={false}>
      <Tabs defaultActiveKey='base'>
          ...
          <Row gutter={[20, 20]} style={{ marginTop: '20px' }}>
            {
              svgNames.map(v => (
                <Col xxl={2} xl={2} lg={4} md={4} sm={6} xs={6} key={v}>
                  <CopyToClipboard text={ '<SvgIcon name="' + v + '" />' } onCopy={(text) => {
        ...
        <TabPane tab='ant design icons' key='ant'>
          <Link href='https://ant.design/components/icon-cn/' target='_blank'>https://ant.design/components/icon-cn/</Link>
        </TabPane>
      </Tabs>
    </Card>
  )
