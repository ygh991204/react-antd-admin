...
const CardHeader = ({ title, data }) => {
  const { t } = useTranslation()
  return (
    <Col xxl={6} xl={6} lg={12} md={12} sm={12} xs={24}>
      <Card bordered={ false }>
        <div style={{ color: 'rgba(0,0,0,.5)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>{ title }</span>
          <Tooltip title={ t('dashboard.intorDes') }>
            <QuestionCircleOutlined style={{ cursor: 'pointer' }}/>
          </Tooltip>
        </div>
        <div style={{ fontSize: '32px', paddingTop: '6px', paddingLeft: '5px' }}>
          { data }
        </div>
      </Card>
    </Col>
